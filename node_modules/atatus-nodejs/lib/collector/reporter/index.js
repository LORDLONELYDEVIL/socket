...
Reporter.prototype._reportAnalytics = function _reportAnalytics(callback) {

  // Here we don't need to verify backend config analytics
  // Because if host info set backend config analytics to false,
  // then analytics payload queue will be there forever.
  // Once backend config analytic set to false, then we will drop in aggregator.
  if (!this.agent._conf.analytics) {
    this.agent.logger.debug("Analytics is not enabled.")
    this.aggregator.resetAnalyticsEvents()
    return process.nextTick(callback)
  }

  if (this.aggregator.isAnalyticsEmpty()) {
    this.agent.logger.debug("No analytic events to send.")
    return process.nextTick(callback)
  }

  let reporter = this
  let payload = this.aggregator.getAndResetAnalyticsPayload();

  let analyticsSanitizedData = []
  let analyticsPayloadLen = 0
  let requests = payload.requests

  if (payload.size > 6 * 1024 * 1024) {
    for (let i = 0; i < requests.length; i++) {
      analyticsSanitizedData.push(requests[i])
      analyticsPayloadLen += (requests[i].bodyLength || 0)

      if (analyticsPayloadLen > 6 * 1024 * 1024) {
        payload.requests = analyticsSanitizedData
        this.transporter.send('analytics', payload, function cb_analytics(error) {
          if (error) reporter.aggregator.mergeAnalyticsEvents(payload)
        })

        analyticsSanitizedData = []
        analyticsPayloadLen = 0
      }
    }
  } else {
    analyticsSanitizedData = requests;
  }

  if (analyticsSanitizedData.length > 0) {
    payload.requests = analyticsSanitizedData
    this.transporter.send('analytics', payload, function cb_analytics(error) {
      if (error) reporter.aggregator.mergeAnalyticsEvents(payload)
