...
var userAgent = 'atatus-apm-node/' + version
...
var confPath = path.resolve(process.env.ATATUS_CONFIG_FILE || 'atatus-config.js')
if (fs.existsSync(confPath)) {
  try {
    var confFile = require(confPath)
  } catch (err) {
    console.error('Atatus agent initialization error: Can\'t read config file %s', confPath)
...
var DEFAULTS = {
  ...
  serverUrl: 'https://apm-rx.atatus.com/track/apm/',
  serverAnalyticsUrl: 'https://an-rx.atatus.com/track/apm/',
  serverHosts: ['apm-rx.atatus.com', 'an-rx.atatus.com'],                      // Only for ignoring

  traceThreshold: 2000,
  ignoreStatusCodes: [ 404 ],
  maxAnalyticsEvents: 10000,
  analytics: false,
  analyticsCaptureOutgoing: false,
  analyticsOutgoingTimeout: 30000,
  analyticsOutgoingOnPatch: false,
  analyticsMask: null,
  analyticsSkip: null,
  skip: null,                     // Function to skip
  backendConfig: {
    analytics: true
  },

  logBodyContentTypes: ["application/json", "text/plain"],                    // Response body captured only these content types
...
var ENV_TABLE = {
  licenseKey: 'ATATUS_LICENSE_KEY',
  appName: 'ATATUS_APP_NAME',
  appVersion: 'ATATUS_APP_VERSION',
  hostname: 'ATATUS_HOSTNAME',
  analytics: 'ATATUS_ANALYTICS',
  analyticsCaptureOutgoing: 'ATATUS_ANALYTICS_CAPTURE_OUTGOING',
  analyticsOutgoingTimeout: 'ATATUS_ANALYTICS_OUTGOING_TIMEOUT',
  analyticsOutgoingOnPatch: 'ATATUS_ANALYTICS_OUTGOING_ON_PATCH',
  maxAnalyticsEvents: 'ATATUS_ANALYTICS_MAX_EVENTS',
  traceThreshold: 'ATATUS_TRACE_THRESHOLD',
  abortedErrorThreshold: 'ATATUS_ABORTED_ERROR_THRESHOLD',
  active: 'ATATUS_ACTIVE',
  addPatch: 'ATATUS_ADD_PATCH',
  apiRequestSize: 'ATATUS_API_REQUEST_SIZE',
  apiRequestTime: 'ATATUS_API_REQUEST_TIME',
  asyncHooks: 'ATATUS_ASYNC_HOOKS',
  captureBody: 'ATATUS_CAPTURE_BODY',
  logBody: 'ATATUS_LOG_BODY',
  captureErrorLogStackTraces: 'ATATUS_CAPTURE_ERROR_LOG_STACK_TRACES',
  captureExceptions: 'ATATUS_CAPTURE_EXCEPTIONS',
  captureHeaders: 'ATATUS_CAPTURE_HEADERS',
  captureSpanStackTraces: 'ATATUS_CAPTURE_SPAN_STACK_TRACES',
  containerId: 'ATATUS_CONTAINER_ID',
  disableInstrumentations: 'ATATUS_DISABLE_INSTRUMENTATIONS',
  environment: 'ATATUS_ENVIRONMENT',
  errorMessageMaxLength: 'ATATUS_ERROR_MESSAGE_MAX_LENGTH',
  errorOnAbortedRequests: 'ATATUS_ERROR_ON_ABORTED_REQUESTS',
  filterHttpHeaders: 'ATATUS_FILTER_HTTP_HEADERS',
  frameworkName: 'ATATUS_FRAMEWORK_NAME',
  frameworkVersion: 'ATATUS_FRAMEWORK_VERSION',
  globalLabels: 'ATATUS_GLOBAL_LABELS',
  instrument: 'ATATUS_INSTRUMENT',
  kubernetesNamespace: ['ATATUS_KUBERNETES_NAMESPACE', 'KUBERNETES_NAMESPACE'],
  kubernetesNodeName: ['ATATUS_KUBERNETES_NODE_NAME', 'KUBERNETES_NODE_NAME'],
  kubernetesPodName: ['ATATUS_KUBERNETES_POD_NAME', 'KUBERNETES_POD_NAME'],
  kubernetesPodUID: ['ATATUS_KUBERNETES_POD_UID', 'KUBERNETES_POD_UID'],
  logLevel: 'ATATUS_LOG_LEVEL',
  logUncaughtExceptions: 'ATATUS_LOG_UNCAUGHT_EXCEPTIONS',
  metricsInterval: 'ATATUS_METRICS_INTERVAL',
  payloadLogFile: 'ATATUS_PAYLOAD_LOG_FILE',
  secretToken: 'ATATUS_SECRET_TOKEN',
  serverTimeout: 'ATATUS_SERVER_TIMEOUT',
  serverUrl: 'ATATUS_SERVER_URL',
  serverAnalyticsUrl: 'ATATUS_SERVER_ANALYTICS_URL',
  serviceName: 'ATATUS_SERVICE_NAME',
  serviceVersion: 'ATATUS_SERVICE_VERSION',
  sourceLinesErrorAppFrames: 'ATATUS_SOURCE_LINES_ERROR_APP_FRAMES',
  sourceLinesErrorLibraryFrames: 'ATATUS_SOURCE_LINES_ERROR_LIBRARY_FRAMES',
  sourceLinesSpanAppFrames: 'ATATUS_SOURCE_LINES_SPAN_APP_FRAMES',
  sourceLinesSpanLibraryFrames: 'ATATUS_SOURCE_LINES_SPAN_LIBRARY_FRAMES',
  stackTraceLimit: 'ATATUS_STACK_TRACE_LIMIT',
  transactionMaxSpans: 'ATATUS_TRANSACTION_MAX_SPANS',
  transactionSampleRate: 'ATATUS_TRANSACTION_SAMPLE_RATE',
  usePathAsTransactionName: 'ATATUS_USE_PATH_AS_TRANSACTION_NAME',
  verifyServerCert: 'ATATUS_VERIFY_SERVER_CERT'
}
...
function config (opts) {
  ...
  opts = Object.assign(
    ...
    confFile, // options read from atatus-apm-node.js config file
    opts, // options passed in to agent.start()
    readEnv() // options read from environment variables
  )

  // Assign log body value to capture body.
  opts.captureBody = opts.logBody || opts.captureBody
  // If analytics option is false, disable captureBody.
  opts.captureBody = opts.analytics ? opts.captureBody : 'off'
  ...
  //       serverAnalyticsUrl: conf.serverAnalyticsUrl,
...
function addClientOptions(opts) {

  // metadata
  opts.agentName = 'nodejs'
  opts.agentVersion = version
  opts.userAgent = userAgent
  opts.nodeVersion = process.version

  opts.environment = opts.releaseStage || opts.environment
  ...
  if (typeof opts.collectErrors !== 'undefined') {
    opts.captureExceptions = !!opts.collectErrors
  }

  if (typeof opts.notifyInterval !== 'undefined') {
    opts.metricsInterval = opts.notifyInterval
  }

  if (opts.notifyHost) {
    opts.serverUrl = (opts.useSSL === false ? 'http://' : 'https://') +
                        opts.notifyHost + (opts.notifyPort ? (':' + opts.notifyPort) : '') +
                        '/track/apm/';
    opts.serverAnalyticsUrl = opts.serverUrl;
  }

  // Force to stop span stacktrace collection
  opts.captureSpanStackTraces = false;

  // Force trace threshold to 100 milliseconds or more
  if (opts.traceThreshold < 100) {
    opts.traceThreshold = 100
  }

  // Force max analytics events to 10,000 if it is more than 10,000
  if (opts.maxAnalyticsEvents > 10000) {
    opts.maxAnalyticsEvents = 10000
  }

  // Check for fn type for both skip and mask
  if (typeof opts.analyticsSkip !== 'function') {
    opts.analyticsSkip = null
  }

  if (typeof opts.analyticsMask !== 'function') {
    opts.analyticsMask = null
  }

  // Check for fn type for skip
  if (typeof opts.skip !== 'function') {
    opts.skip = null
  }

  // Force metrics interval to 60 seconds or more
  if (opts.metricsInterval < 60) {
    opts.metricsInterval = 60
  }
