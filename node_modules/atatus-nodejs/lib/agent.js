...
Agent.prototype._config = function (opts) {
  ...
  // this._conf.serverHosts = [ url.host ]
...
Agent.prototype.start = function (opts) {
  ...
  this._config(opts)
  this._transport = new Collector(this)
  ...
  if (!this._conf.active) {
    this.logger.info('Atatus agent is disabled due to configuration')
    return this
  } else if (!this._conf.apiKey) {
    if (!this._conf.appName && !this._conf.licenseKey) {
      this.logger.error('Atatus agent isn\'t correctly configured: Missing appName and licenseKey.')
      this._conf.active = false
      return this
    } else if (!this._conf.appName) {
      this.logger.error('Atatus agent isn\'t correctly configured: Missing appName.')
      this._conf.active = false
      return this
    } else if (!this._conf.licenseKey) {
      this.logger.error('Atatus agent isn\'t correctly configured: Missing licenseKey.')
      this._conf.active = false
      return this
    }

  // } else if (!/^[a-zA-Z0-9 _-]+$/.test(this._conf.serviceName)) {
  //   this.logger.error('Atatus agent isn\'t correctly configured: serviceName "%s" contains invalid characters! (allowed: a-z, A-Z, 0-9, _, -, <space>)', this._conf.serviceName)
  //   this._conf.active = false
  //   return this
  // } else if (this._conf.serverPort < 1 || this._conf.serverPort > 65535) {
  //   this.logger.error('Atatus agent isn\'t correctly configured: serverUrl "%s" contains an invalid port! (allowed: 1-65535)', this._conf.serverUrl)
  //   this._conf.active = false
  //   return this
...
Agent.prototype.startMonitor = function () {
  // Dummy function
  // WebPack build removes the atatus agent if it is imported from start script directly and use environment and atatus config file for configurations.
  // Then you should use "atatus.startMonitor()" to avoid removing atatus agent import.
  return true
}
...
Agent.prototype.setCompanyContext = function (context) {
  var trans = this.currentTransaction
  if (!trans) return false
  if (context && context.id) {
    context.id = '' + context.id
  }
  trans.setCompanyContext(context)
  return true
}
...
Agent.prototype.setResponseBody = function (responseBody) {
  var trans = this.currentTransaction
  if (!trans) return false
  if (!responseBody) return false
  responseBody = parsers.getBodyStr(responseBody)
  trans.setResponseBody(responseBody)
  return true
}

Agent.prototype.setUser = Agent.prototype.setUserContext
Agent.prototype.setCompany = Agent.prototype.setCompanyContext
Agent.prototype.setCustomData = Agent.prototype.setCustomContext

Agent.prototype.setTags = function (tags) {
  return this.addLabels(tags)
}

Agent.prototype.setSkip = function (fn) {
  if (typeof fn !== 'function') {
    this.logger.error('Can\'t add skip of type %s', typeof fn)
    return
  }
...
Agent.prototype.setAnalyticsSkip = function (fn) {
  if (typeof fn !== 'function') {
    this.logger.error('Can\'t add analytics skip of type %s', typeof fn)
    return
  }

  this._conf.analyticsSkip = fn
}

Agent.prototype.setAnalyticsMask = function (fn) {
  if (typeof fn !== 'function') {
    this.logger.error('Can\'t add analytics mask of type %s', typeof fn)
    return
  }

  this._conf.analyticsMask = fn
}

Agent.prototype.setLogBodyContentTypes = function (contentType) {
  if (!contentType) return this.logger.warn('Content type is empty!')

  if (Array.isArray(contentType)) {
    this._conf.logBodyContentTypes = contentType
  } else if (typeof contentType === 'string') {
    this._conf.logBodyContentTypes.push(contentType)
  }
}
...
Agent.prototype.captureError = function (err, opts, cb) {
  ...
  function prepareError (error) {
    ...
    if (trans) {
      ...
      error.transaction_name = trans.name
      error.transaction = {
        type: trans.type,
    ...
    if (req) {
      error.context.request = parsers.getContextFromRequest(req, agent._conf)
    }

    if (res) {
      error.context.response = parsers.getContextFromResponse(res, agent._conf)
    }
  ...
  function send (error) {
    ...
    if (agent._transport) {
      // agent.logger.info(`Sending error to Atatus APM`, { id: error.id })
...
Agent.prototype.handleUncaughtExceptions = function (cb) {
  ...
  this._uncaughtExceptionListener = function (err) {
    ...
    agent.logger.debug('Atatus agent caught unhandled exception: %s', err.message)
...
Agent.prototype.startBackgroundTransaction = function (name, opts) {
  return this.startTransaction(name, 'background', opts)
}
...
Agent.prototype.notifyError = function (err, opts) {
  opts = opts || {}
  opts.custom = opts.customData
  this.captureError(err, opts)
}

Agent.prototype.excludeURL = function (pattern) {
  if (!pattern) return this.logger.warn('URL pattern is empty!')

  if (typeof pattern === 'string') this._conf.ignoreUrlStr.push(pattern)
  else this._conf.ignoreUrlRegExp.push(pattern)
}
