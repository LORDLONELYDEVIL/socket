...
const TEST = process.env.ATATUS_TEST
...
function Span (transaction, name, type, opts) {
  ...
  if (transaction._agent._instrumentation.activeSpan) {
    this.level = (transaction._agent._instrumentation.activeSpan.level || 0) + 1
  } else {
    this.level = 1
  }
...
Span.prototype._encode = function (cb) {
  ...
  function done (err, frames) {
    ...
    var startOffset = 0
    if (self._timer && self.transaction._timer) {
        startOffset = (self._timer.start - self.transaction._timer.start) / 1000
    ...
    if (self._db || self._labels || self._http) {
      payload.context = {
...
function filterCallsite (callsite) {
  ...
  return filename ? filename.indexOf('/node_modules/atatus-nodejs/') === -1 : true
}
